...
matches <- read_gage_info(type = 'matches')
...
metrics_labels <- metric_labeller(metrics_sel)
...
if(set == 'all'){gage_info <- rbind(hw_gage_info, ds_gage_info)
                 set_label <- 'Headwaters + Downstream'}

if(set == 'ds_connected'){gage_info <- ds_connected_gage_info
                           set_label <- 'Downstream Connections'}

if(set == 'ds_matched'){gage_info <- ds_matched_gage_info
                        set_label <- 'Downstream Matches'}

if(set == 'hw2'){gage_info <- filter(hw_gage_info, order %in% 1:2)
...
for(i in 1:length(metrics_sel)){
  ...
  ggsave(paste0('figures/data_maps/metric_trends/sigs/',set,'/',metrics_sel[i],'_',sig_var,'.png'), height = 3.6, width = 6, units = 'in')
}
