function [x_Q_meanduration, error_flag, error_str] = sig_x_Q_meanduration(Q, t, type, varargin)
...
switch type
    ...
    case 'no'
        ...
        if isempty(interval_lengths)
            ...
            x_Q_meanduration = mean(interval_lengths);
        end
    ...
    case 'custom_high'
        ...
        x_Q_meanduration = NaN(length(threshold),1);
        for t = 1:length(threshold)
            ...
            interval_lengths = end1 - start1 + 1;
            if isempty(interval_lengths)
                x_Q_meanduration(t) = 0;
            else
                x_Q_meanduration(t) = mean(interval_lengths);
            end
        end
