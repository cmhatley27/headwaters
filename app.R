...
trend_dat <- read_csv('data/gages/metrics/trends/metrics_trends_window3.csv') %>%
  group_by(var) %>%
  ...
  mutate(x = st_coordinates(geometry)[,1],
         y = st_coordinates(geometry)[,2])
cn_dat <- read_csv('data/gages/cn_diffs.csv') %>%
  mutate(type = 'headwater') %>%
  st_as_sf(coords = c('lon', 'lat'), crs = 4269) %>%
  st_transform(5070) %>%
  mutate(x = st_coordinates(geometry)[,1],
         y = st_coordinates(geometry)[,2])
metric_dat <- read_csv('data/gages/metrics/merged/metrics_window3.csv', col_select = !contains('error_str')) %>%
...
server <- function(input, output) {
  ...
  observeEvent(point_sel(), ignoreNULL = F, {
    ...
    if(length(points_sel) > length(points_sel_prev)){
      ...
      loc <- filter(gage_loc, site_no %in% new_gage_point) %>%
        mutate(symbol_val = case_when(
          type == 'headwater' ~ 100,
  ...
  # details tab -------------------------------------------------------------
  #display list of selected gages
  output$selected_gages <- renderUI({
    text <- c('Selected gages:', point_sel())
    HTML(paste(text, sep = '<br/>'))
