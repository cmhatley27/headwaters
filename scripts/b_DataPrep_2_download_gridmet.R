...
hw_gage_info <- read_csv('data/gages/hw_gage_info_exp.csv') #read_gage_info(type = 'headwaters')
ds_gage_info <- read_csv('data/gages/ds_gage_info_exp.csv') #read_gage_info(type = 'downstream')
ds_match_gage_info <- read_csv('data/gages/ds_matched_gage_info_exp.csv') #read_gage_info('downstream_matched')
...
for(i in 1:nrow(boundaries)){
  ...
  save_path <- paste0('./data/gages/climate_exp/',gage_id,'.csv')
...
#03079000 temp
bad_temp <- read_csv('./data/gages/climate_exp/03079000.csv') %>%
...
write_csv(temp_fix_save, './data/gages/climate_exp/03079000.csv')
...
bad_pet1 <- read_csv('./data/gages/climate_exp/03360500.csv') %>%
  mutate(pet = NA)
write_csv(bad_pet1, './data/gages/climate_exp/03360500.csv')
bad_pet2 <- read_csv('./data/gages/climate_exp/08202700.csv') %>%
  mutate(pet = NA)
write_csv(bad_pet2, './data/gages/climate_exp/08202700.csv')
