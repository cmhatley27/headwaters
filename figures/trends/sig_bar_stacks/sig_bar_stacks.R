...
# plot # sigs by metric ---------------------------------------------------
site <- 'ds' #hw or ds or ds_connected or ds_matched
orders <- 1:100
sig <- 'sig_ar2a' #sig_ar for autocorrelation-corrected p<0.05, _ar2a for corrected p<0.1, _0 for non-corrected p<0.05
save <- T
fig_height <- 5 #7.5 works for 16 metrics, 4 for 6 metrics
fig_width <- 6 #seems to be good for all metric sets
fig_title <- 'Trend significance at downstream gages'
...
fig_name <- paste0(site,'_',sig,'.png')

if(site == 'hw') site_sel <- hw_gage_info
if(site == 'ds') site_sel <- ds_gage_info
if(site == 'all') site_sel <- all_gage_info
if(site == 'ds_connected') site_sel <- ds_connected_gage_info
if(site == 'ds_matched') site_sel <- ds_matched_gage_info
site_sel <- filter(site_sel, order %in% orders)$site_no

trends_fil <- filter(trends, site_no %in% site_sel & var %in% metrics_sel) %>%
  mutate(across(contains('sig_'), ~replace_na(.x, 'none')))

trends_by_metric <- as.data.frame(table(select(trends_fil, var, trend = sig))) %>%
...
if(save == T){
  ggsave(paste0('figures/trends/sig_bar_stacks/',fig_name), height = fig_height, width = fig_width, units = 'in')
}
