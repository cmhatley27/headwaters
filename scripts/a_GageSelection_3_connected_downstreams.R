...
fil_gages_ds <- filter(fil_gages_ds, site_no %nin% hw_gage_info$site_no)

hw_as_ds <- sum(hw_ds_connections$downstream_id %in% hw_gage_info$site_no)
print(paste0('There are ',nrow(hw_ds_connections),' total connections involving ',
             length(unique(hw_ds_connections$headwater_id)),' unique headwater gages and ',
             nrow(fil_gages_ds), ' unique downstream gages. ',
             hw_as_ds, ' of these connections are between two headwater gages, leaving ',
             nrow(hw_ds_connections)-hw_as_ds, ' connections between a headwater and a true downstream'))
