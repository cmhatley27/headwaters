...
thresholds_high = [3 6 9 12 15]; % flow thresholds (as multiples of median) for calculating duration and frequency over/under
thresholds_low = [0.75 0.67 0.5 0.33 0.25];
...
for file = 1:length(files)
    ...
    for year = 1:(length(years)-window_size+1)
        ...
        try
            ...
            for i = 1:length(fieldnames)
                ...
                elseif endsWith(fieldnames{i}, '_low') % Ensure freq/duration threshold metrics are arrays of length = thresholds
                    sigs.(fieldnames{i}) = NaN(1, length(thresholds_low));
                else
    ...
    results.site_no = repmat(site, (length(years)-window_size+1), 1); %repmat(site, (length(years)/window_size), 1);%
    results.wateryear = years(1:(length(years)-window_size+1)).'; %years((((1:length(years)/window_size)-1)*window_size)+1).';%
    
    writetable(results, strcat("./data/gages/metrics/loose/window_",num2str(window_size),"/",site,".csv"))
    disp(strcat("gage ",num2str(file),"/",num2str(length(files))," done!"))
end

%% merge
%window_size = 3;
files = ls(strcat("./data/gages/metrics/loose/window_",num2str(window_size),"/*.csv"));
paths = strcat("./data/gages/metrics/loose/window_",num2str(window_size),"/",files);
read_opts = detectImportOptions(paths(1));
read_opts = setvartype(read_opts,"site_no","string");
read_opts = setvartype(read_opts,strcmp(read_opts.VariableTypes,'char'),"string");

all_dat = table('Size',[length(files)*(length(years)-window_size+1) length(read_opts.VariableNames)],...%table('Size',[length(files)*(length(years)/window_size) length(read_opts.VariableNames)],...%
...
for file = 1:length(files)
    ...
    all_dat((file-1)*(length(years)-window_size+1)+1:(file)*(length(years)-window_size+1),:) = dat;%all_dat((file-1)*(length(years)/window_size)+1:file*(length(years)/window_size),:) = dat; %
end

writetable(all_dat, strcat("./data/gages/metrics/merged/metrics_window",num2str(window_size),".csv"))